...
export function ControllerList({ initialControllers }: { initialControllers: Controller[] }) {
  ...
  const toggleApproval = async (controller: Controller) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Unable to update controller approval mode.");
      setControllers(current => current.map(item => item.id === controller.id ? { ...item, controllerApprovalRequired: Boolean(data.approvalRequired) } : item));
      setMessage(data.approvalRequired ? "Controller approval is now required for quiz actions." : "Controller approval is no longer required.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to update controller approval mode.");
    } finally {
      setBusyAction("");
    }
  };
  ...
  return <>
    ...
    <div className="grid gap-4">{controllers.length > 0 ? controllers.map((controller) => <div key={controller.id} className="rounded-lg border border-slate-200 bg-white p-6"><div className="mb-4 flex items-start justify-between"><div><h3 className="text-lg font-bold text-slate-900">{controller.name}</h3><div className="mt-2 flex flex-wrap gap-4 text-sm text-slate-600"><span>QuizNexa ID: {controller.quiznexaId || "Not assigned"}</span><span className="flex items-center gap-1"><Calendar className="h-4 w-4" />Assigned {new Date(controller.createdAt).toLocaleDateString()}</span></div></div><div className="text-right text-sm"><p><b>{controller.createdQuizzes.length}</b> quizzes created</p><p><b>{controller.completedTasks}</b> tasks completed</p><p><b>{controller.averageRating.toFixed(1)}</b> / 5 rating</p></div></div>
      ...
      <div className="flex flex-wrap gap-3 border-t border-slate-200 pt-4"><Link href={`/admin/controllers/${controller.id}`} className="flex-1 rounded bg-blue-50 py-2 text-center font-medium text-blue-700">View details</Link><Link href={`/admin/controllers/${controller.id}/tasks`} className="flex-1 rounded bg-indigo-50 py-2 text-center font-medium text-indigo-700">Assign tasks</Link><button type="button" disabled={busyAction === `approval:${controller.id}`} onClick={() => void toggleApproval(controller)} className={`flex-1 rounded py-2 font-medium text-white disabled:opacity-60 ${controller.controllerApprovalRequired ? "bg-emerald-600" : "bg-red-600"}`}>{busyAction === `approval:${controller.id}` ? "Saving..." : controller.controllerApprovalRequired ? "Approval required" : "Require approval"}</button><button type="button" onClick={() => setPendingRemoval(controller)} className="flex-1 rounded bg-red-50 py-2 font-medium text-red-700">Remove</button></div>
