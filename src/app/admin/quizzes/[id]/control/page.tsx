...
type QuizData = { id: string; title: string; runtimeStatus: string; isActive: boolean; quizType: string; visits: { id: string; userId: string | null; name: string; status: string; joinedAt: string }[]; teams: { id: string; name: string; members: { user: { id: string; name: string; email: string } }[] }[]; attempts: { id: string; score: number; user?: { id: string; name: string; email: string } | null; team?: { id: string; name: string } | null }[] };
...
export default function QuizControlPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const liveParticipants = ["STOPPED", "COMPLETED"].includes(quiz.runtimeStatus)
    ? []
    : quiz.visits.filter((visit, index, visits) => ["JOINED", "ENTERED"].includes(visit.status) && (!visit.userId || visits.findIndex((candidate) => candidate.userId === visit.userId) === index));
