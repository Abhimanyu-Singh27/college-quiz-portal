...
export default async function ControllerLayout({ children }: { children: React.ReactNode }) {
  ...
  return <><StaffSidebar role="CONTROLLER" user={{ name: user?.name || session.name, quiznexaId: user?.quiznexaId || session.quiznexaId || null, joinedAt: user?.createdAt.toISOString() || null }} /><PortalRefreshButton /><div className="lg:pl-64">{children}</div></>;
}
